 ...
 *    외부에서만 선언이 가능합니다. 클래스 내부라면 어떤 위치든 선언이
 *    가능합니다.
 * 3. 메서드 내부에서는 다른 메서드의 호출만 가능합니다.
 * 4. 호출하는 과정은 메서드를 사용하는 과정이며 해당 메서드가 필요로 하는
 *    데이터(매개값)를 메서드내부로 전달하면서 해당 메서드는 매개값을 통해
...
public class MethodExample1 {
	...
	public static int calcSum2(int begin, int end) {
		...
		int sum = 0; //short sum=0; 가능 왜냐면, short가 int보다 낮기때문에 자동으로 형변환이 되서 들어간다. double sum=0; 일 경우, return (int)sum;
